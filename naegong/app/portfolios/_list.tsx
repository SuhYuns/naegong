...
export default function PortfolioListClient() {
  ...
  useEffect(() => {
    if (!session?.user) {
      router.replace('/login?next=/portfolio'); // 기존 동작 유지
      return;
    }
    ...
    // params는 useSearchParams() 이므로 클라이언트에서만 유효. Suspense로 감싸져 있으니 OK.
